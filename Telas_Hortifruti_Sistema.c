...
void PuxarFuncionarios(MYSQL* conn) {
    ...
    if (mysql_query(conn, "SET NAMES 'utf8mb4'") || mysql_query(conn, "SET CHARACTER SET utf8mb4")) {
        fprintf(stderr, "Erro ao definir conjunto de caracteres: %s\n", mysql_error(conn));
        return;
    }
...
int main() {
    ...
    do {
        ...
        if (strcmp(selecionar_opcao, "compra") == 0) {
            ...
            else if (strcmp(selecionar_opcao, "funcionarios") == 0) {
                do {
                    system("cls");
                    setConsoleColors(15, 0);
                    printf("Digite seu login:");
                    ...
                    if (acesso == 1 && acesso2 == 1) {
                        ...
                        do {
                            ...
                            if (strcmp(selecionar_opcao, "s") == 0) {
                                ...
                                printf("\n");
                                system("cls");
                                printf("Deseja adicionar senha e login de acesso para o funcionário? [S/N]\n");

                                char opcao_login[2];
                                scanf_s("%2s", opcao_login, (unsigned)_countof(opcao_login));
                                ParaMinúsculas(opcao_login);

                                if (strcmp(opcao_login, "s") == 0) {
                                    char login_novo[30];
                                    char senha_nova[30];

                                    system("cls");
                                    printf("Digite o login que deseja adicionar: \n");
                                    fflush(stdin);
                                    scanf_s("%29s", login_novo, (unsigned)_countof(login_novo));

                                    printf("Digite a senha que deseja adicionar: \n");
                                    fflush(stdin);
                                    scanf_s("%29s", senha_nova, (unsigned)_countof(senha_nova));

                                    snprintf(query, sizeof(query), "INSERT INTO CREDENCIAIS_ACESSO (LOGIN, SENHA) VALUES ('%s', '%s')", login_novo, senha_nova);

                                    if (mysql_query(conn, query)) {
                                        fprintf(stderr, "Erro na consulta: %s\n", mysql_error(conn));
                                    }
                                    else {
                                        printf("Login e senha adicionados com sucesso!\n");
                                    }
                                }
                                else {
                                    limite = 3;
